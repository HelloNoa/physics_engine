...
export class Sprite implements I_Sprite{
    ...
    GetSimpleCircle(): {radius: number, middle: Vector} {
        let mid: Vector = new Vector2();
        let radius: number = 0;
    ...
    GetSpecificCircle(i: number, j: number, k: number): {radius: number, middle: Vector} {
        let middle: Vector2 = new Vector2();
        let radius: number = 0;

        // 세 점의 삼각형 외심(삼각형 모든 변의 수직이등분선 교점) 계산
        const mid1: Vector = new Vector2().Divide(new Vector2().Plus(this.position[i], this.position[j]), 2);
        const mid2: Vector = new Vector2().Divide(new Vector2().Plus(this.position[j], this.position[k]), 2);
    ...
    IsOuterCircle(mid: Vector, rad: number): boolean {//vector number
